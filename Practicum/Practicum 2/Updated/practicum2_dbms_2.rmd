...
SELECT pubmed_article.pmid,pubmed_article.medlinecitation_statusid,pubmed_article.article_pubmodel_ID, pubmed_article.article_id, journal.issn, journal.publication_year, journal.publication_month FROM pubmed_article INNER JOIN JOURNAL using (article_id);
...
SELECT pubmed_article.pmid,pubmed_article.medlinecitation_statusid,pubmed_article.article_pubmodel_ID, pubmed_article.article_id, journal.issn, journal.publication_year, author.author_id, author.first_name, author.last_name, count(distinct(pubmed_article.article_id)) FROM pubmed_article INNER JOIN JOURNAL using (article_id) INNER JOIN author using (article_id) group by first_name, publication_year;
...
select * from author_summary_fact_table limit 5;
...
SELECT pubmed_article.pmid,pubmed_article.medlinecitation_statusid,pubmed_article.article_pubmodel_ID, pubmed_article.article_id, journal.publication_year, count(distinct(journal.issn)) FROM pubmed_article INNER JOIN JOURNAL using (article_id) INNER JOIN author using (article_id) group by publication_year;

```

```{sql connection = dbcon}

select * from journal_summary_fact_table limit 5;

```

Part 3 (20 pts) Explore and Mine Data

3a. (10 pts) Write queries using your data warehouse to explore whether the publications show a seasonal pattern. Look beyond the pattern of number of publications per season. Adjust your fact tables as needed to support your new queries. If you need to update the fact table, document your changes and your reasons why the changes are needed.
...
select article_pubmodel_name, count(pmid) from transaction_fact_table t INNER JOIN  article_pubmodel a
using (article_pubmodel_ID) where t.article_pubmodel_ID = a.article_pubmodel_ID group by article_pubmodel_ID;

```

```{sql connection=dbcon}

select article_pubmodel_name, publication_month, count(pmid) from transaction_fact_table t INNER JOIN  article_pubmodel a
using (article_pubmodel_ID) where t.article_pubmodel_ID = a.article_pubmodel_ID group by article_pubmodel_ID, publication_month;

```

```{sql connection=dbcon}

SELECT * from transaction_fact_table;

```


3b. (10 pts) Either (a) visualize (graph/plot) the data from the previous step using R to explore seasonality and explain what you found, or (b) build a predictive model to forecast the expected number of publications for a quarter. (Note that we do not cover predictive modeling in this course, so if you do not know this from a prior course, then create the visualization.) For the visualization you may use either the plotting functions of Base R or the more sophisticated functions of ggplot.
...
transaction_distribution_pubmodelName <- dbGetQuery(dbcon, "select article_pubmodel_name, count(pmid) as count from transaction_fact_table t INNER JOIN  article_pubmodel a
using (article_pubmodel_ID) where t.article_pubmodel_ID = a.article_pubmodel_ID group by article_pubmodel_ID;")

head(transaction_distribution_pubmodelName)

ggplot(data = transaction_distribution_pubmodelName, mapping=aes(x=article_pubmodel_name, y=count))+ 
...
transaction_distribution_pubmodelmonth <- dbGetQuery(dbcon, "select article_pubmodel_name, publication_month, count(pmid) as count from transaction_fact_table t INNER JOIN  article_pubmodel a
using (article_pubmodel_ID) where t.article_pubmodel_ID = a.article_pubmodel_ID group by article_pubmodel_ID, publication_month;")

ggplot(data = transaction_distribution_pubmodelName) + 
geom_bar(mapping=aes(x=article_pubmodel_name, y=publication_month), stat = "identity", fill=rgb(0.1,0.4,0.5,0.7))
